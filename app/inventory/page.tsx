                                        ...
                                        {isLoadingLogs ? (
                                            ...
                                            <div className="relative space-y-1">
                                                ...
                                                {(() => {
                                                    ...
                                                    return materialLogs.map((log, index, arr) => {
                                                        const isLast = index === arr.length - 1;
                                                        const nextLog = arr[index + 1];
                                                        
                                                        const getDotStyle = (l: typeof log | undefined) => {
                                                            if (!l) return { fill: "bg-slate-200 dark:bg-slate-800", hollow: "border-2 border-slate-200 bg-white dark:bg-slate-950" };
                                                            const isUpd = l.quantityChanged === 0;
                                                            const isMvIn = l.actionType === 'import' && !!l.parentLog;
                                                            const isMvOut = l.actionType === 'withdraw' && moveSourceIds.has(l._id);
                                                            const isMv = isMvIn || isMvOut;
                                                            
                                                            if (isUpd) return { fill: "bg-slate-300 dark:bg-slate-600", hollow: "border-2 border-slate-300 bg-white dark:bg-slate-950" };
                                                            if (isMv) return { fill: "bg-violet-500", hollow: "border-2 border-violet-500 bg-white dark:bg-slate-950" };
                                                            if (l.actionType === 'import') return { fill: "bg-emerald-500", hollow: "border-2 border-emerald-500 bg-white dark:bg-slate-950" };
                                                            if (l.actionType === 'cut') return { fill: "bg-blue-500", hollow: "border-2 border-blue-500 bg-white dark:bg-slate-950" };
                                                            if (l.actionType === 'claim') return { fill: "bg-red-500", hollow: "border-2 border-red-500 bg-white dark:bg-slate-950" };
                                                            return { fill: "bg-orange-500", hollow: "border-2 border-orange-500 bg-white dark:bg-slate-950" };
                                                        };
                                                        
                                                        const currentStyle = getDotStyle(log);
                                                    ...
                                                    return (
                                                        <div key={log._id} className="relative flex items-start gap-3 pl-1 py-2.5 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-900/60 transition-colors">
                                                            {/* Vertical connecting line */}
                                                            {!isLast && <div className={`absolute left-[10px] top-[21px] h-[calc(100%+4px)] w-0.5 z-0 ${lineStyle}`} />}

                                                            {/* Timeline dot */}
                                                            <div className={`mt-1 h-3.5 w-3.5 rounded-full shrink-0 z-10 ${index === 0 ? currentStyle.hollow : `border-2 border-white dark:border-slate-950 ${currentStyle.fill}`}`} />
