...
export function OrderReleasePanel({
    ...
    const [acknowledged, setAcknowledged]   = useState<Record<string, Set<VerifyKey>>>({});
    ...
    const getOrderChecks = (order: Order) => {
        ...
        const fieldStatus = (bill: string | null, mismatch: boolean): FieldStatus => {
            if (bill == null) return "no_data";
            return mismatch ? "mismatch" : "match";
        };

        const checks: { key: VerifyKey; label: string; bill: string | null; sys: string | null; status: FieldStatus }[] = [
            { key: "customer", label: "ลูกค้า",        bill: billCus,                                                sys: ordCus,        status: fieldStatus(billCus, cusMismatch)  },
            { key: "quantity", label: "จำนวน",         bill: billQty != null ? `${billQty} ${mUnit}` : null,           sys: `${ordQty} ${mUnit}`, status: fieldStatus(billQty != null ? String(billQty) : null, qtyMismatch)  },
            { key: "type",     label: "ประเภทสินค้า",  bill: billType,                                                sys: ordType,       status: fieldStatus(billType, typeMismatch) },
            { key: "price",    label: "ราคาประมาณ",   bill: billPrice != null ? `${billPrice.toLocaleString()} ฿` : null, sys: null, status: billPrice != null ? "match" : "no_data" },
        ];
        ...
        return { checks, hasBill, billDeadline, deadlineWarning, cusMismatch, qtyMismatch, typeMismatch };
    };
    ...
    const canRelease = (order: Order): { ok: boolean; reasons: string[] } => {
        ...
        if (unackedMismatches.length > 0) {
            reasons.push(`ยังไม่ได้รับทราบข้อมูลไม่ตรง: ${unackedMismatches.map(c => c.label).join(", ")}`);
        }
    ...
    const buildSummary = (order: Order) => {
        const { checks, hasBill, billDeadline, deadlineWarning } = getOrderChecks(order);
        const curSt = assignments[order._id] ?? [];
        const stationMap = new Map(stations.map(s => [s._id, s.name]));
        const mId = matId(order.material);
        ...
        checks.filter(c => c.status === "mismatch").forEach(c => {
        ...
        else if (showStockCheck && st === "low") warnings.push(`สต็อกไม่พอ (มี ${stock}, ต้องการ ${order.quantity})`);
        if (deadlineWarning === "past") warnings.push(`เลยกำหนดส่ง (${new Date(billDeadline!).toLocaleDateString("th-TH")})`);
        else if (deadlineWarning === "soon") warnings.push(`ใกล้กำหนดส่ง (${new Date(billDeadline!).toLocaleDateString("th-TH")})`);
        ...
        {confirmDialog && (() => {
            ...
            return (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setConfirmDialog(null)}>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl max-w-md w-full p-6 space-y-5" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-xl bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center">
                                <Rocket className="h-5 w-5 text-violet-600 dark:text-violet-400" />
                            </div>
                            <div>
                                <h3 className="text-base font-bold text-slate-900 dark:text-white">ยืนยันปล่อยงาน</h3>
                                <p className="text-xs text-slate-500">ตรวจสอบข้อมูลก่อนกดยืนยัน</p>
                            </div>
                        </div>
                        ...
                        <div className="rounded-xl border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800 overflow-hidden">
                            <div className="px-4 py-2.5 flex justify-between text-sm">
                                ...
                                <span className="font-medium text-slate-900 dark:text-white">{summary.customer}</span>
                            </div>
                            <div className="px-4 py-2.5 flex justify-between text-sm">
                                <span className="text-slate-500">วัสดุ</span>
                                <span className="font-medium text-slate-900 dark:text-white">{summary.material}</span>
                            </div>
                            <div className="px-4 py-2.5 flex justify-between text-sm">
                                <span className="text-slate-500">จำนวน</span>
                                <span className="font-medium text-slate-900 dark:text-white">{summary.quantity} {summary.unit}</span>
                            </div>
                            <div className="px-4 py-2.5 text-sm">
                                <span className="text-slate-500">สถานี ({summary.stationNames.length})</span>
                                <div className="flex flex-wrap gap-1.5 mt-1.5">
                                    {summary.stationNames.map((name, i) => (
                                        <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-300 text-xs font-medium">
                                            <span className="text-[10px] text-violet-400">{i + 1}.</span> {name}
                                        </span>
                                    ))}
                                </div>
                            </div>
                        ...
                        {summary.warnings.length > 0 && (
                            <div className="rounded-xl border border-amber-200 dark:border-amber-800/50 bg-amber-50/50 dark:bg-amber-950/20 p-3 space-y-1.5">
                                <div className="flex items-center gap-1.5 text-amber-700 dark:text-amber-400">
                                    ...
                                    <span className="text-xs font-semibold">คำเตือนที่รับทราบแล้ว</span>
                                </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {releasing === confirmDialog._id
                                    ? <><Loader2 className="h-4 w-4 animate-spin" /> กำลังปล่อย...</>
            ...
            {loading ? (
                ...
                <div className="divide-y divide-border/40">
                    {orders.map(order => {
                        ...
                        const { checks, hasBill, billDeadline, deadlineWarning, cusMismatch, qtyMismatch, typeMismatch } = getOrderChecks(order);
                                ...
                                {isOpen && (
                                    <div className="px-4 pb-4 pt-3 border-t border-border/30 bg-muted/10 space-y-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-1.5 mb-2">
                                                ...
                                                {hasBill && billDeadline && (
                                                    <span className={`ml-auto text-[10px] flex items-center gap-1 ${
                                                        deadlineWarning === "past" ? "text-red-600 dark:text-red-400 font-semibold" :
                                                        deadlineWarning === "soon" ? "text-amber-600 dark:text-amber-400 font-semibold" :
                                                        "text-muted-foreground"
                                                    }`}>
                                                        {deadlineWarning && <Clock className="h-3 w-3" />}
                                                        {deadlineWarning === "past" ? "เลยกำหนดส่ง! " : deadlineWarning === "soon" ? "ใกล้กำหนดส่ง " : "กำหนดส่ง: "}
                                                        {new Date(billDeadline).toLocaleDateString("th-TH", { day: "numeric", month: "short", year: "2-digit" })}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            {/* ── No-bill warning box ── */}
                                            {!hasBill && (
                                                <div className={`rounded-xl border p-3 mb-3 ${noBillAcked.has(order._id)
                                                    ...
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">ออเดอร์นี้ไม่มีบิล/ใบสั่งอ้างอิง</p>
                                                            <p className="text-[11px] text-amber-700 dark:text-amber-400 mt-0.5">ไม่สามารถเปรียบเทียบข้อมูลกับใบสั่งได้</p>
                                                        </div>
                                                        {!noBillAcked.has(order._id) ? (
                                                            <button
                                                                type="button"
                                                                onClick={() => setNoBillAcked(prev => new Set([...prev, order._id]))}
                                            ...
                                            {deadlineWarning === "past" && (
                                                <div className={`rounded-xl border p-3 mb-3 ${deadlineAcked.has(order._id)
                                                    ...
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-xs font-semibold text-red-800 dark:text-red-300">
                                                                เลยกำหนดส่งแล้ว ({new Date(billDeadline!).toLocaleDateString("th-TH")})
                                                            </p>
                                                            <p className="text-[11px] text-red-700 dark:text-red-400 mt-0.5">ออเดอร์นี้เลยกำหนดส่ง ต้องรับทราบก่อนปล่อยงาน</p>
                                                        </div>
                                                        {!deadlineAcked.has(order._id) ? (
                                                            <button
                                                                type="button"
                                                                onClick={() => setDeadlineAcked(prev => new Set([...prev, order._id]))}
                                            ...
                                            {/* ── Stock warning ── */}
                                            {showStockCheck && stock <= 0 && (
                                                <div className={`rounded-xl border p-3 mb-3 ${stockAcked.has(order._id)
                                                    ? "border-red-300/50 dark:border-red-800/30 bg-red-50/30 dark:bg-red-950/10"
                                                    : "border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/30"
                                                }`}>
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-xs font-semibold text-red-800 dark:text-red-300">สต็อกวัสดุหมด</p>
                                                            <p className="text-[11px] text-red-700 dark:text-red-400 mt-0.5">
                                                                {matName(order.material)} — ต้องการ {order.quantity} {mUnit} แต่สต็อกเป็น 0
                                                            </p>
                                                        </div>
                                                        {!stockAcked.has(order._id) ? (
                                                            <button
                                                                type="button"
                                                                onClick={() => setStockAcked(prev => new Set([...prev, order._id]))}
                                                                className="shrink-0 px-3 py-1.5 rounded-lg bg-red-600 text-white text-[11px] font-semibold hover:bg-red-700 transition-colors"
                                                            >
                                                                รับทราบ
                                                            </button>
                                            ...
                                            <div className="rounded-xl border border-border/60 overflow-hidden">
                                                ...
                                                <div className="grid grid-cols-[4rem_1fr_1fr_2.5rem] items-center gap-2 px-3 py-1.5 bg-muted/40 border-b border-border/40">
                                                    ...
                                                    <span className="text-[10px] font-semibold text-muted-foreground text-center">สถานะ</span>
                                                </div>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex justify-center">
                                                                ...
                                                                {status === "no_data" && bill == null && sys != null && (
                                            ...
                                            {hasBill && checks.some(c => c.status === "mismatch" && !acked.has(c.key)) && (
                                                <p className="mt-2 text-[11px] text-red-600 dark:text-red-400 flex items-center gap-1">
                                                    <AlertTriangle className="h-3.5 w-3.5" />
                                                    กดช่องว่างสีแดงเพื่อรับทราบรายการที่ข้อมูลไม่ตรง ({checks.filter(c => c.status === "mismatch" && !acked.has(c.key)).length} รายการ)
                                                </p>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 pt-1">
                                            ...
                                            {!isReleasable && (
                                                <div className="flex-1 min-w-0 space-y-0.5">
                                                    {reasons.slice(0, 2).map((r, i) => (
                                                        <span key={i} className="flex items-center gap-1 text-[11px] text-amber-600 dark:text-amber-400 font-medium">
                                                            <AlertTriangle className="h-3 w-3 shrink-0" />
                                                            <span className="truncate">{r}</span>
                                                        </span>
                                                    ))}
                                                    {reasons.length > 2 && (
                                                        <span className="text-[10px] text-muted-foreground">+{reasons.length - 2} รายการ</span>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            <div className={isReleasable ? "ml-auto" : ""}>
                                                <button
                                                    ...
                                                    title={!isReleasable ? reasons.join("\n") : undefined}
