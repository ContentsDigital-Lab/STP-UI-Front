...
import { Database, ChevronRight, ChevronLeft, ChevronDown, Loader2, AlertCircle, Hash, QrCode as QrCodeIcon, FileText, Package, Printer } from "lucide-react";
...
export function RecordList({
    ...
    /** Fetch ALL pages from a paginated API endpoint (max 100/page, up to 5 pages = 500 records) */
    const fetchAllPages = async (endpoint: string): Promise<Record<string, unknown>[]> => {
        const sep = endpoint.includes("?") ? "&" : "?";
        const limit = dataSource === "/panes" ? 500 : 100;
        const firstRes = await fetchApi<{ success: boolean; data: Record<string, unknown>[]; pagination?: { totalPages: number } }>(`${endpoint}${sep}limit=${limit}&page=1`);
        if (!firstRes.success) return [];
        const allData = [...(firstRes.data ?? [])];
        const totalPages = firstRes.pagination?.totalPages ?? 1;
        if (totalPages > 1) {
            const remaining = Array.from({ length: Math.min(totalPages, 5) - 1 }, (_, i) =>
                fetchApi<{ success: boolean; data: Record<string, unknown>[] }>(`${endpoint}${sep}limit=${limit}&page=${i + 2}`)
            );
            const results = await Promise.all(remaining);
            for (const r of results) {
                if (r.success && r.data) allData.push(...r.data);
            }
        }
        return allData;
    };
    ...
    const loadData = (silent = false) => {
        ...
        const baseUrl = (shouldFilterStation && dataSource === "/orders")
            ? `/orders?stationId=${encodeURIComponent(stationId!)}`
            : dataSource;

        const fetchMain = fetchAllPages(baseUrl);
    ...
    const needsPagination = capped.length > PAGE_SIZE;
    const listTotalPages = needsPagination ? Math.max(1, Math.ceil(capped.length / PAGE_SIZE)) : 1;
    const safePage = Math.min(listPage, listTotalPages);
    const filtered = needsPagination
        ? capped.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE)
        : capped;

    // Reset page when search/data changes
    useEffect(() => { setListPage(1); }, [query, rows]);
            ...
            <div className="w-full rounded-xl border bg-card overflow-hidden">
                ...
                {showHeader && (
                    <div className="flex items-center justify-between px-4 py-2.5 border-b bg-muted/30">
                        ...
                        {fetching
                            ...
                            : <span className="text-[10px] text-muted-foreground">{capped.length} รายการ</span>
                        }
                    </div>
                )}
                {/* Search */}
                                                                ...
                                                                const paneContent = (
                                                                    <>
                                                                        ...
                                                                        {pane.laminateRole === "sheet" && pane.sheetLabel && (
                                                                            <span className="text-[8px] font-bold px-1 py-0.5 rounded bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 shrink-0">{pane.sheetLabel}</span>
                                                                        )}
                ...
                {listTotalPages > 1 && (
                    <div className="flex items-center justify-between px-4 py-2.5 border-t bg-muted/10">
                        <span className="text-[10px] text-muted-foreground">
                            {safePage} / {listTotalPages} ({capped.length} รายการ)
                        </span>
                        <div className="flex items-center gap-0.5">
                            <button
                                disabled={safePage <= 1}
                                onClick={() => setListPage(p => Math.max(1, p - 1))}
                                className="h-7 w-7 rounded-md flex items-center justify-center text-muted-foreground hover:bg-muted disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                            >
                                <ChevronLeft className="h-3.5 w-3.5" />
                            </button>
                            {Array.from({ length: Math.min(listTotalPages, 5) }, (_, i) => {
                                let pg: number;
                                if (listTotalPages <= 5) pg = i + 1;
                                else if (safePage <= 3) pg = i + 1;
                                else if (safePage >= listTotalPages - 2) pg = listTotalPages - 4 + i;
                                else pg = safePage - 2 + i;
                                return (
                                    <button
                                        key={pg}
                                        onClick={() => setListPage(pg)}
                                        className={`h-7 w-7 rounded-md flex items-center justify-center text-[11px] font-medium transition-colors ${
                                            safePage === pg
